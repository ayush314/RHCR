...
class AggregateResultsTests(unittest.TestCase):
    def test_hierarchical_effects_preserve_layout_pairing(self) -> None:
        ...
        for layout in (2, 3):
            for seed in (6, 7):
                for method, value in (("pibt_pressure_aware", 1.0), ("pibt_phase_aware", 0.0)):
                    rows.append({
    ...
    def test_failed_status_without_summary_counts_as_zero_yield(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            cell = root / "cell"
            cell.mkdir()
            (cell / "status.json").write_text(json.dumps({
                "map": "alley",
                "agent_count": 60,
                "method": "pbs_phase_aware",
                "seed": 6,
                "status": "failed",
                "failure_reason": "wall_timeout",
            }))
            rows = aggregate_results.read_status_rows(root)
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0]["clean_completion"], 0.0)
        self.assertEqual(rows[0]["service_rate"], 0.0)
