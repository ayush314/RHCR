...
MAPFSolver* set_solver(const BasicGraph& G, const boost::program_options::variables_map& vm)
{
	...
	else if (solver_name == "PIBT2")
	{
		PIBT2* pibt = new PIBT2(G, *path_planner);
		pibt->set_pibt_policy(vm["pibt_policy"].as<string>());
		pibt->random_tiebreak = vm["pibt_random_tiebreak"].as<bool>();
		mapf_solver = pibt;
	}
...
int main(int argc, char** argv) 
{
	...
	desc.add_options()
		...
		("screen,s", po::value<int>()->default_value(1), "screen option (0: none; 1: results; 2:all)")
			("solver", po::value<string>()->default_value("PBS"), "solver (LRA, PBS, PIBT, PIBT2, WHCA, ECBS)")
    ...
    if (scenario == "WORKSTATION")
    {
        ...
        if ((workstation_solver != "PBS" && workstation_solver != "PIBT" &&
             workstation_solver != "PIBT2") || vm["id"].as<bool>())
        {
            std::cerr << "WORKSTATION requires --solver PBS, --solver PIBT, or --solver PIBT2 with --id false" << endl;
            exit(-1);
        }
        string station_policy = vm["station_policy"].as<string>();
        string pibt_policy = vm["pibt_policy"].as<string>();
        if ((workstation_solver == "PBS" && !is_valid_workstation_policy(station_policy)) ||
    ...
    else if (scenario == "WORKSTATION")
    {
        ...
        if ((solver->get_name() == "PIBT" || solver->get_name() == "PIBT2") &&
            G.has_shared_sortation_heuristics())
